...
app.post("/sensor", async (req, res) => {
  try {
    ...
    const settings = settingsResult.rows[0] || {
      ...
      do_min: 5.0, water_level_min: 10.0, water_level_max: 100.0,
      ammonia_max: 0.5
    };
    ...
    if (doVal < settings.do_min) {
      alerts.push({ parameter: "Dissolved Oxygen", old_value: "Low", new_value: doVal });
    }
    ...
    if (ammVal > settings.ammonia_max) {
      alerts.push({ parameter: "Ammonia", old_value: "High", new_value: ammVal });
    }
    ...
    if (existing.rows.length > 0) {
      result = await pool.query(
        `UPDATE sensor_settings SET 
          ...
          do_min = $5, water_level_min = $6, water_level_max = $7, ammonia_max = $8
        WHERE id = $9 RETURNING *`,
        [temp_min, temp_max, ph_min, ph_max, do_min, water_level_min, water_level_max, ammonia_max, existing.rows[0].id]
      );
    } else {
      result = await pool.query(
        `INSERT INTO sensor_settings (temp_min, temp_max, ph_min, ph_max, do_min, water_level_min, water_level_max, ammonia_max)
        VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *`,
        [temp_min, temp_max, ph_min, ph_max, do_min, water_level_min, water_level_max, ammonia_max]
      );
    }

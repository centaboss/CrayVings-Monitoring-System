...
const char* ssid = "AIRLINK_MADUCDOC-2.4G";
const char* password = "64j91g6d";
...
// Watchdog timeout — must be longer than worst-case WiFi reconnect
const int WDT_TIMEOUT_SEC = 60;

// WiFi reconnect will give up after this many ms to avoid watchdog reset
const int WIFI_CONNECT_TIMEOUT_MS = 20000;

// pH calibration — based on pH 7 buffer solution measurement
// Formula: ph = PH_PHASE_SHIFT * voltage + PH_CALIBRATION_OFFSET
// Adjust PH_CALIBRATION_OFFSET if readings drift (dip probe in pH 7 solution and tune)
...
float readPH() {
    ...
    // Average middle 6 samples (discard 2 lowest and 2 highest)
...
// --- WiFi (FIX: timeout so watchdog never triggers) ---
...
bool connectWiFi() {
    ...
    while (WiFi.status() != WL_CONNECTED) {
        // Reset watchdog while waiting so it doesn't trigger
        esp_task_wdt_reset();

        if (millis() - startTime >= WIFI_CONNECT_TIMEOUT_MS) {
            Serial.println("\nWi-Fi connection timed out! Will retry next loop.");
            return false;
        }
...
void setup() {
    ...
    // ESP32 Arduino core v3.x uses a config struct for watchdog init
...
void loop() {
    ...
    // Read ultrasonic distance and convert to water level
    ...
    // Validate each sensor individually with specific warnings
    bool tempOK  = isTemperatureValid(tempC);
    bool levelOK = isWaterLevelValid(waterLevel);
    bool phOK    = isPHValid(phValue);

    // Only send if ALL sensors are valid
    if (WiFi.status() == WL_CONNECTED) {
        if (tempOK && levelOK && phOK) {
            HTTPClient http;
            ...
            Serial.println("Data not sent: one or more sensor readings invalid (see warnings above).");
        }
    } else {
